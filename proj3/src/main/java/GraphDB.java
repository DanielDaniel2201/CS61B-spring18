...
public class GraphDB {
     ...
     */
    private static final Map<Long, Node> vertices = new LinkedHashMap<>();
     ...
     */
    private void clean() {
        vertices.entrySet().removeIf(item -> item.getValue().adj.isEmpty());
    }
    ...
    long closest(double lon, double lat) {
        ...
        for (Node node : vertices.values()) {
            double current = distance(node.lon, node.lat, lon, lat);
            if (current < closest) {
                closest = current;
                id = node.id;
            }
    ...
    void addEdge(Long n1, Long n2) {
        vertices.get(n1).adj.add(n2);
        vertices.get(n2).adj.add(n1);
    }

    void addEdges(ArrayList<Node> nodes) {
        for (int i = 0; i < nodes.size() - 1; i += 1) {
            addEdge(nodes.get(i).id, nodes.get(i + 1).id);
        }
    }

    void removeNode(Node n) {
        vertices.remove(n.id, n);
    }
